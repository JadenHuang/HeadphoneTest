...
class CSRSPIDevice(with_metaclass(Singleton, object)):
    ...
    def __getLib(self):
        if __name__ == "__main__":
             ...
             dll_path = os.path.join(_DIRNAME, "CSR_BT")
    ...
    def openTestEngineSpi(self):
        port = ctypes.wintypes.DWORD(-1)
...
#
    ...
    def psRead(self, deviceHandle, pskeyID, storeMask, arrayLen, dataArray):
        ...
        pskeySize = self.psSize(self.spiHandle,pskeyID, storeMask)
...
if __name__ == "__main__":
    ...
    dataArray = []
    csrapi.psRead(csrapi.spiHandle,0x0001, PS_STORES_R, 4, dataArray)
    print ("psSize of 0x0001 is {}".format(csrapi.psSize(csrapi.spiHandle,0x0001,PS_STORES_IFR)))
    print dataArray
    dataArray[3] = 1
    csrapi.psWrite(csrapi.spiHandle,0x0001, PS_STORES_T, 4, dataArray)
    dataArray = []
    csrapi.psRead (csrapi.spiHandle,0x0001, PS_STORES_T, 4, dataArray)
    print dataArray
    
    # Test psReadBdAddr
    lap, uap, nap = csrapi.psReadBdAddr(csrapi.spiHandle)
    print ("lap = {}, uap = {}, nap = {}".format(lap, uap, nap))

    # Test bccmdSetWarmReset
    csrapi.bccmdSetWarmReset(csrapi.spiHandle)
    print "bccmdSetWarmReset successful"
    dataArray = []
    csrapi.psRead (csrapi.spiHandle,0x0001, PS_STORES_T, 4, dataArray)
    print dataArray

    csrapi.closeTestEngine(csrapi.spiHandle)
